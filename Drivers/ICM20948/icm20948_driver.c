...
//         snprintf((char*)message, sizeof(message), "Handle is correctly set to
//         &hi2c1\n");
//     } else {
//         snprintf((char*)message, sizeof(message), "Handle is NOT set
//         correctly!\n");
//     }
//     HAL_UART_Transmit(&huart2, message, strlen((char*)message),
//     HAL_MAX_DELAY);
// }

ICM20948 GET_ICM20948_Device(ICM20948_Serial_If* sif) {
    ICM20948 device = {.serial_if = sif, .current_bank = 0x00};
    return device;
}
...
uint8_t ICM20948_Update_Bank(ICM20948* device, uint16_t reg) {
    ...
    if (bank != device->current_bank) {
        ...
        device->serial_if->write_reg(device->serial_if->context, REG_BANK_SEL, &bank, 1);
    }
    return address;
}
...
ICM20948_STATUS ICM20948_WHO_AM_I(ICM20948* device) {
    ...
    uint8_t id = 0xFF;
    if (device->serial_if->read_reg(device->serial_if->context, address, &id,
                                    1)) {
        return ICM20948_STATUS_ERROR;
    }
    return ICM20948_STATUS_SUCCESS;

    // if (rc != HAL_OK) {
    //     char error_msg[] = "Error: Unable to read IMU ID!\r\n";
    //     HAL_UART_Transmit(&huart2, (uint8_t*)error_msg, sizeof(error_msg) -
    //     1, HAL_MAX_DELAY); return -1;
    // }

    // char msg[50];
    // if (id == WHOAMI) {
    //     snprintf(msg, sizeof(msg), "IMU ID successfully read: 0x%02X\r\n",
    //     id); print_message_uart2(msg); return 0;
    // } else {
    //     snprintf(msg, sizeof(msg), "IMU ID mismatch! Read: 0x%02X\r\n", id);
    //     HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg),
    //     HAL_MAX_DELAY); return -1;
    // }
}
...
void ICM20948_Set_Accel_Config(ICM20948* device, enum dlpfcfg dlpf_cfg,
    ...
    uint8_t config_val =
        CONSTRUCT_ACCEL_GYRO_CONFIG(fchoice, accel_fs, dlpf_cfg);
    uint8_t addr = ICM20948_Update_Bank(device, (uint16_t)REG_ACCEL_CONFIG);
    device->serial_if->write_reg(device->serial_if->context, addr, &config_val,
                                 sizeof(config_val));
    switch (accel_fs) {
        case MPU_FS_2G:
...
void ICM20948_Set_Gyro_Config(ICM20948* device, enum dlpfcfg dlpf_cfg,
                              ...
                              enum FCHOICE_STATE fchoice) {
    uint8_t config_val =
        CONSTRUCT_ACCEL_GYRO_CONFIG(fchoice, gyro_fs, dlpf_cfg);
...
void ICM20948_READ_ACCEL_GYRO_DATA(ICM20948* device) {
    ...
    device->serial_if->read_reg(device->serial_if->context, reg_addr,
                                raw_buffer, 12);
    ...
    values->accel_gyro_data[0] =
        accel_raw_x / device->configuration.accel_sensitivity;
    values->accel_gyro_data[1] =
        accel_raw_y / device->configuration.accel_sensitivity;
    values->accel_gyro_data[2] =
        accel_raw_z / device->configuration.accel_sensitivity;

    float gyro_bias[3] = {0.0, 0.0, 0.0};
    // float gyro_bias[3] = {-0.11428002f, -0.64344996f, 0.88612f};
    values->accel_gyro_data[3] =
        gyro_raw_x / device->configuration.gyro_sensitivity - gyro_bias[0];
    values->accel_gyro_data[4] =
        gyro_raw_y / device->configuration.gyro_sensitivity - gyro_bias[1];
    values->accel_gyro_data[5] =
        gyro_raw_z / device->configuration.gyro_sensitivity - gyro_bias[2];
...
void ICM20948_Calculate_YawPitchRoll(ICM20948* device) {
    ...
    current->angle_accel[0] =
        atan2(current->accel_gyro_data[2], current->accel_gyro_data[1]) * 180 /
        M_PI;  // Pitch
    current->angle_accel[1] =
        atan2(-current->accel_gyro_data[0], current->accel_gyro_data[1]) * 180 /
        M_PI;  // Roll

    // Time difference in seconds
    double dt = (current->timestamp - previous->timestamp) / 1000.0;

    current->angle_gyro[0] =
        previous->angle_gyro[0] + current->accel_gyro_data[3] * dt;
    current->angle_gyro[1] =
        previous->angle_gyro[1] + current->accel_gyro_data[4] * dt;

    // Fuse angles
    double alpha = 0.98;
    current->fused_angles[0] = alpha * current->angle_gyro[0] +
                               (1 - alpha) * current->angle_accel[0];  // Pitch
    current->fused_angles[1] = alpha * current->angle_gyro[1] +
                               (1 - alpha) * current->angle_accel[1];  // Roll
}
...
void ICM20948_Calculate_Mean(ICM20948* device) {
    ...
    for (uint8_t i = 0; i < IMU_BUFFER_SIZE; i++) {
        ...
        double meana = 0.0;
        meana = (device->imuBuffer->buffer[i].accel_gyro_data[0] *
                 device->imuBuffer->buffer[i].accel_gyro_data[0]) +
                (device->imuBuffer->buffer[i].accel_gyro_data[1] *
                 device->imuBuffer->buffer[i].accel_gyro_data[1]) +
                (device->imuBuffer->buffer[i].accel_gyro_data[2] *
                 device->imuBuffer->buffer[i].accel_gyro_data[2]);
        ...
        double meang = 0.0;
        meang = (device->imuBuffer->buffer[i].accel_gyro_data[3] *
                 device->imuBuffer->buffer[i].accel_gyro_data[3]) +
                (device->imuBuffer->buffer[i].accel_gyro_data[4] *
                 device->imuBuffer->buffer[i].accel_gyro_data[4]) +
                (device->imuBuffer->buffer[i].accel_gyro_data[5] *
                 device->imuBuffer->buffer[i].accel_gyro_data[5]);
    ...
    double var = (sum_of_squares_a / (double)IMU_BUFFER_SIZE) -
                 (overall_mean_a * overall_mean_a);
    var = sqrt(var);
